...
# fetch-conf.sh - Fetch configuration files from various sources
#
# DESCRIPTION:
#   This script supports multiple methods to fetch configuration files.
#   Currently supports: git repositories, HTTP/HTTPS downloads
#
# USAGE:
#   fetch-conf.sh git [options] <repo_url> <src_path> <dest_path>
#   fetch-conf.sh wget [options] <url> <dest_path>
#
# OPTIONS:
...
#
# EXAMPLES:
#   fetch-conf.sh git git@github.com:user/configs.git server.conf /etc/liteflow.conf
#   fetch-conf.sh git --backup https://github.com/user/configs.git client.conf ./liteflow.conf
#   fetch-conf.sh wget https://example.com/liteflow.conf /etc/liteflow.conf
#   fetch-conf.sh wget --backup https://configs.example.com/server.json ./liteflow.conf
...
fetch_wget() {
    ...
    log "Successfully fetched configuration from URL"
    return 0
}
...
fetch_git() {
    ...
    if [ -d "$git_tmp_dir/.git" ]; then
        ...
        git fetch origin --depth=1 || {
            log "Error: Failed to fetch from git repo"
            cd - >/dev/null || true
            return 1
        }
    ...
    log "Successfully fetched configuration from git repo"
    return 0
}
...
usage() {
    ...
    log "Subcommands:" >&2
    log "  git     Fetch configuration from git repository" >&2
    log "  wget    Fetch configuration from HTTP/HTTPS URL" >&2
    log "" >&2
